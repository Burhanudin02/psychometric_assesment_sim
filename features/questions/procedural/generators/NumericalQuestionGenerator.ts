...
export class NumericalQuestionGenerator implements IProceduralQuestionGenerator {
  readonly domain = "NUMERICAL_REASONING";
  readonly questionType = QuestionType.NUMERICAL_MCQ;
  readonly subtopics = ["percentages"] as const;

  generate(seed?: number): QuestionItem {
    const rand = seed ? Math.sin(seed) * 10000 : Math.random();
    const base = 100 + Math.floor(Math.abs(rand * 80) % 80) * 10;
    const pct = [10, 15, 20, 25, 30][Math.floor(Math.abs(rand * 5) % 5)];
    ...
    return {
      ...
      solvingStrategy:
        "Kalikan langsung harga normal dengan faktor pengali desimal (1 - persen diskon).",
